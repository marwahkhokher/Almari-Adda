...
function ChipGroup({ options, selected, onToggle, label, icon: Icon, index }) {
  return (
    <motion.div
      ...
      className="rounded-2xl border border-[#e6d5b8] bg-white shadow-sm p-6 mb-5"
    >
      <div className="flex items-center gap-2 mb-4">
        <div className="w-8 h-8 rounded-lg bg-[#f3e6cf] flex items-center justify-center">
          <Icon className="w-4 h-4 text-[#7a2331]" />
        </div>
        <p className="text-xs font-semibold text-[#8a7360] uppercase tracking-wider">
          {label}
        </p>
      </div>
          ...
          <motion.button
            ...
            className={`px-4 py-2 rounded-full text-xs font-medium capitalize border transition-colors ${
              selected.includes(opt)
                ? 'bg-[#7a2331] border-[#7a2331] text-white shadow-sm'
                : 'bg-white border-[#e6d5b8] text-[#3d2417] hover:border-[#7a2331]'
            }`}
          >
            {opt}
          </motion.button>
    ...
    <div className="min-h-screen bg-[#FBF3E7]">
      ...
      <div className="fixed inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b border-[#e6d5b8] bg-[#FBF3E7]/95 px-4 backdrop-blur lg:hidden">
        ...
        <button type="button" onClick={() => setIsProfileOpen(true)} className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-[#e6d5b8] bg-white">
          {avatarUrl ? <img src={avatarUrl} alt={firstName} className="h-full w-full object-cover" /> : <User size={17} />}
        </button>
      </div>
        ...
        <main className="flex-1 px-4 md:px-10 py-8 max-w-[1440px] w-full mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-4 mb-8">
            ...
            <div className="text-center">
              ...
              <p className="text-[#8a7360] text-sm">Pick what you&rsquo;re feeling, and we&rsquo;ll pull it from your closet.</p>
            </div>
          ...
          {/* FORM CARD — same border/shadow/radius treatment as VisualizeScreen's main card */}
          <div className="bg-[#FCF6EC] rounded-3xl border border-[#e6d5b8] shadow-[0_8px_30px_rgba(61,36,23,0.08)] p-4 md:p-6 max-w-7xl mx-auto">
            ...
            <motion.div custom={3} initial="hidden" animate="visible" variants={sectionVariants}>
              <motion.button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl text-sm font-semibold bg-[#7a2331] hover:bg-[#631b28] text-white transition-colors disabled:opacity-50 shadow-sm"
              >
            ...
            <AnimatePresence mode="wait">
              ...
              {message && (
                <motion.p
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="mt-4 text-sm text-[#8a7360] text-center bg-[#f3e6cf]/70 rounded-lg py-2 px-3"
                >
                  {message}
                </motion.p>
              )}
                ...
                >
                  {result.type === 'top_bottom' ? (
                    <div className="grid grid-cols-2 gap-5">
                      {[result.top, result.bottom].map((item, i) => (
                        <motion.div
                          ...
                          whileHover={{ y: -6 }}
                          className="border border-[#e6d5b8] rounded-2xl p-4 bg-white shadow-sm"
                        >
                          <img src={item.image_url} alt={item.subcategory} className="w-full aspect-square object-contain" />
                          <p className="text-xs text-center mt-3 capitalize text-[#8a7360]">{item.subcategory}</p>
                        </motion.div>
                      ))}
                    </div>
                  ) : (
                    <motion.div
                      ...
                      whileHover={{ y: -6 }}
                      className="max-w-[240px] mx-auto border border-[#e6d5b8] rounded-2xl p-4 bg-white shadow-sm"
                    >
                      <img src={result.item.image_url} alt={result.item.subcategory} className="w-full aspect-square object-contain" />
                      <p className="text-xs text-center mt-3 capitalize text-[#8a7360]">{result.item.subcategory}</p>
                    </motion.div>
                  )}
                  ...
                  {hasAlternatives && (
                    <motion.button
                      ...
                      disabled={isGenerating}
                      className="mt-5 w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold border border-[#7a2331] text-[#7a2331] hover:bg-[#7a2331]/5 transition-colors disabled:opacity-50"
                    >
      ...
      <button
        ...
        className="
          ...
          bg-[#7a2331]
          text-white
          shadow-[0_10px_30px_rgba(122,35,49,0.30)]
          transition-all
          duration-200
          hover:-translate-y-1
          hover:bg-[#631b28]
          hover:shadow-[0_14px_34px_rgba(122,35,49,0.38)]
        "
      >
