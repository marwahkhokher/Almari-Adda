...
export default function ClosetScreen() {
  ...
  const fetchCatalogue = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Could not load your wardrobe from backend. Please ensure backend is running.');
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    >
      ...
      <div className="p-3 border-t border-surface-light bg-white flex justify-between items-center">
        <div>
          ...
          <p className="text-text-muted text-xs capitalize">{item.category}</p>
        </div>
        {item.confidence && (
          <span className="text-[10px] text-accent-hover bg-pastel-pink px-2 py-0.5 rounded-full font-semibold border border-accent-light/50">
